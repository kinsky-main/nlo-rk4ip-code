...
)
{
    ...
    if (state->nonlinear_model == NLO_NONLINEAR_MODEL_KERR_RAMAN) {
        ...
        const nlo_operator_eval_context eval_ctx = {
            ...
            .ky_grid = state->tensor_mode_active ? state->working_vectors.ky_mesh_vec : NULL,
            .t_grid = state->tensor_mode_active ? state->working_vectors.t_mesh_vec : NULL,
            .x_grid = state->tensor_mode_active ? state->working_vectors.x_mesh_vec : NULL,
            .y_grid = state->tensor_mode_active ? state->working_vectors.y_mesh_vec : NULL,
            .field = field,
            .dispersion_factor = state->working_vectors.dispersion_factor_vec,
            .potential = state->working_vectors.potential_vec,
            .half_step_size = state->current_half_step_exp
