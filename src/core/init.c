...
static size_t count_working_full_volume_vectors(const simulation_state* state)
{
    if (state == NULL) {
        return 0u;
    }
    ...
    if (state->fft_plan != NULL &&
        vector_backend_get_type(state->backend) == VECTOR_BACKEND_VULKAN) {
        if (count == SIZE_MAX) {
            return SIZE_MAX;
        }
        count += 1u;
...
static int estimate_working_vector_bytes(
...
)
{
    ...
    const size_t full_count = count_working_full_volume_vectors(state);
    ...
    if (checked_mul_size_t(per_record_bytes, full_count, &full_bytes) != 0) {
        return -1;
    }
...
static void fill_allocation_info(
...
{
    if (out_allocation_info == NULL || state == NULL || exec_options == NULL) {
        return;
    }
    ...
    if (state->field_buffer != NULL) {
        ...
        if (checked_mul_size_t(per_record_bytes, state->num_host_records, &field_buffer_bytes) == 0) {
            host_snapshot_bytes += field_buffer_bytes;
        }
    }
    if (estimate_working_vector_bytes(per_record_bytes, state, &working_bytes) != 0) {
        working_bytes = 0u;
    }

    out_allocation_info->requested_records = requested_records;
    out_allocation_info->allocated_records = (state->snapshot_scratch_record != NULL) ? 1u : state->num_host_records;
    out_allocation_info->per_record_bytes = per_record_bytes;
    out_allocation_info->host_snapshot_bytes = host_snapshot_bytes;
    out_allocation_info->working_vector_bytes = working_bytes;
    out_allocation_info->device_ring_capacity = state->record_ring_capacity;
    out_allocation_info->device_budget_bytes = exec_options->forced_device_budget_bytes;
    if (vector_backend_get_type(state->backend) == VECTOR_BACKEND_VULKAN) {
        ...
        if (vec_query_memory_info(state->backend, &mem_info) == VEC_STATUS_OK) {
            ...
            effective_budget =
                (effective_budget / DEVICE_RING_BUDGET_HEADROOM_DEN) *
                DEVICE_RING_BUDGET_HEADROOM_NUM;
            out_allocation_info->device_budget_bytes = effective_budget;
        }
    }
    out_allocation_info->backend_type = exec_options->backend_type;
}
...
)
{
    ...
    *out_state = NULL;
    if (out_allocation_info != NULL) {
        *out_allocation_info = (allocation_info){0};
    }
    ...
    const execution_options local_options =
        (exec_options != NULL)
            ? *exec_options
    ...
    fill_allocation_info(num_time_samples,
                              num_recorded_samples,
                              state,
                              &local_options,
                              out_allocation_info);
