...
enum {
    VK_DESCRIPTOR_SET_BUDGET_DEFAULT_BYTES = 512u * 1024u,
    VK_DESCRIPTOR_SET_ESTIMATED_BYTES = 512u,
    VK_DESCRIPTOR_SET_MIN_COUNT = 16u,
    VK_DESCRIPTOR_SET_MAX_COUNT = 4096u
};

static uint32_t vk_clamp_descriptor_set_count(uint64_t count)
{
    if (count < (uint64_t)VK_DESCRIPTOR_SET_MIN_COUNT) {
        return VK_DESCRIPTOR_SET_MIN_COUNT;
    }
    if (count > (uint64_t)VK_DESCRIPTOR_SET_MAX_COUNT) {
        return VK_DESCRIPTOR_SET_MAX_COUNT;
    }
    return (uint32_t)count;
}

static uint32_t vk_select_descriptor_set_target(const vk_backend_config* config)
{
    if (config != NULL && config->descriptor_set_count_override > 0u) {
        return vk_clamp_descriptor_set_count((uint64_t)config->descriptor_set_count_override);
    }

    size_t budget_bytes = VK_DESCRIPTOR_SET_BUDGET_DEFAULT_BYTES;
    if (config != NULL && config->descriptor_set_budget_bytes > 0u) {
        budget_bytes = config->descriptor_set_budget_bytes;
    }

    uint64_t estimated_count = (uint64_t)(budget_bytes / (size_t)VK_DESCRIPTOR_SET_ESTIMATED_BYTES);
    if (estimated_count == 0u) {
        estimated_count = (uint64_t)VK_DESCRIPTOR_SET_MIN_COUNT;
    }

    return vk_clamp_descriptor_set_count(estimated_count);
}

static bool vk_supports_required_features(
...
static uint32_t vk_find_memory_type(
...
)
{
    ...
    uint32_t type_index = vk_find_memory_type(backend, mem_req.memoryTypeBits, properties);
...
void vk_destroy_buffer_raw(vector_backend* backend, VkBuffer* buffer, VkDeviceMemory* memory)
{
    if (backend == NULL) {
        return;
    }
...
static vec_status vk_create_command_resources(vector_backend* backend, const vk_backend_config* config)
{
...
static vec_status vk_create_descriptor_resources(
    vector_backend* backend,
    const vk_backend_config* config
)
{
    ...
    VkPushConstantRange push_range = {
        ...
        .offset = 0u,
        .size = (uint32_t)sizeof(vk_push_constants)
    };
    ...
    uint32_t target_count = vk_select_descriptor_set_target(config);
    while (target_count >= VK_DESCRIPTOR_SET_MIN_COUNT) {
        ...
        if (target_count == VK_DESCRIPTOR_SET_MIN_COUNT) {
            break;
        }
        uint32_t next_count = target_count / 2u;
        if (next_count < VK_DESCRIPTOR_SET_MIN_COUNT) {
            next_count = VK_DESCRIPTOR_SET_MIN_COUNT;
        }
        fprintf(stderr,
                "[vk] descriptor set allocation failed at %u sets; retrying at %u sets.\n",
                target_count,
                next_count);
    ...
    if (backend->vk.descriptor_set_count == 0u || backend->vk.descriptor_sets == NULL) {
        fprintf(stderr,
                "[vk] failed to allocate descriptor sets in range [%u, %u].\n",
                VK_DESCRIPTOR_SET_MIN_COUNT,
                vk_select_descriptor_set_target(config));
        return VEC_STATUS_BACKEND_UNAVAILABLE;
    }
...
static void vk_destroy_resources(vector_backend* backend)
{
    if (backend == NULL) {
        return;
    }
    ...
    vk_destroy_buffer_raw(backend, &backend->vk.staging_buffer, &backend->vk.staging_memory);
    vk_destroy_buffer_raw(backend, &backend->vk.reduction_buffer_a, &backend->vk.reduction_memory_a);
    vk_destroy_buffer_raw(backend, &backend->vk.reduction_buffer_b, &backend->vk.reduction_memory_b);
...
vec_status vk_ensure_staging_capacity(vector_backend* backend, VkDeviceSize min_bytes)
{
    ...
    if (capacity == 0u) {
        capacity = (VkDeviceSize)VK_DEFAULT_STAGING_BYTES;
    }
    ...
    vec_status status = vk_create_buffer_raw(
        backend,
        capacity,
    ...
    if (vkMapMemory(backend->vk.device, new_memory, 0u, capacity, 0u, &mapped) != VK_SUCCESS) {
        vk_destroy_buffer_raw(backend, &new_buffer, &new_memory);
        return VEC_STATUS_BACKEND_UNAVAILABLE;
    }
...
vec_status vk_ensure_reduction_capacity(vector_backend* backend, VkDeviceSize min_elements)
{
    ...
    status = vk_create_buffer_raw(
        ...
        &mem_b);
    if (status != VEC_STATUS_OK) {
        vk_destroy_buffer_raw(backend, &new_a, &mem_a);
        return status;
    }
...
vec_status vk_backend_init(vector_backend* backend, const vk_backend_config* config)
{
    ...
    if (!vk_supports_required_features(config->physical_device, &limits)) {
        return VEC_STATUS_BACKEND_UNAVAILABLE;
    }
    if (limits.maxPushConstantsSize < sizeof(vk_push_constants)) {
        return VEC_STATUS_BACKEND_UNAVAILABLE;
    }
    if (limits.maxComputeWorkGroupInvocations < VK_LOCAL_SIZE_X ||
        limits.maxComputeWorkGroupSize[0] < VK_LOCAL_SIZE_X ||
    ...
    vec_status status = vk_create_command_resources(backend, config);
    if (status != VEC_STATUS_OK) {
        vk_destroy_resources(backend);
        return status;
    }

    status = vk_create_descriptor_resources(backend, config);
    if (status != VEC_STATUS_OK) {
        vk_destroy_resources(backend);
        return status;
    }

    status = vk_create_kernels(backend);
    if (status != VEC_STATUS_OK) {
        vk_destroy_resources(backend);
        return status;
    }

    VkDeviceSize initial_staging = vk_min_size((VkDeviceSize)VK_DEFAULT_STAGING_BYTES, backend->vk.max_kernel_chunk_bytes);
    if (initial_staging == 0u) {
        initial_staging = (VkDeviceSize)VK_DEFAULT_STAGING_BYTES;
    }
    status = vk_ensure_staging_capacity(backend, initial_staging);
    if (status != VEC_STATUS_OK) {
        vk_destroy_resources(backend);
        return status;
    }
...
void vk_backend_shutdown(vector_backend* backend)
{
    ...
    if (backend->vk.simulation_phase_recording) {
        (void)vk_simulation_phase_flush(backend);
    }
