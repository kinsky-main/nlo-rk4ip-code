...
image_tag="${NLO_DOCKER_CUDA_IMAGE:-nlolib-cuda-dev}"
build_dir="${NLO_DOCKER_BUILD_DIR:-build-docker-cuda}"

if [[ "${NLO_DOCKER_VALIDATE_DRY_RUN:-0}" != "0" ]]; then
  echo "[nlolib] docker_validate_cuda.sh dry-run"
  echo "image_tag=${image_tag}"
  echo "build_dir=${build_dir}"
  exit 0
fi

docker build -t "${image_tag}" -f docker/cuda-dev.Dockerfile .
docker run --rm \
  --gpus all \
  ...
  bash -lc "cmake -S . -B ${build_dir} -GNinja \
    ...
    echo "[nlolib] docker_validate_cuda.sh skipped: CUDA Docker runtime or GPUs unavailable."
  }
