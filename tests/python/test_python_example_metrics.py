...
from backend.metrics import (  # type: ignore
...
def test_pointwise_abs_relative_error_formula() -> None:
    ...
    got = relative_l2_intensity_error(pred, ref, eps=1e-12, warn_on_clamp=False)
...
def test_relative_l2_intensity_error_curve_reduction() -> None:
    pred = np.asarray(
        [
            [2.0, 5.0, 1.0],
            [3.0, 1.0, 3.0],
    ...
    curve = relative_l2_intensity_error_curve(pred, ref, eps=1e-12, warn_on_clamp=False)
    ...
    total = relative_l2_intensity_error(pred, ref, eps=1e-12, warn_on_clamp=False)
...
def test_near_zero_reference_clamp_warning() -> None:
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        got = relative_l2_intensity_error(pred, ref, eps=1e-6, warn_on_clamp=True, context="unit_test")
    if len(caught) <= 0:
...
def main() -> None:
    ...
    test_relative_l2_intensity_error_curve_reduction()
