...
static void test_fft_backend_selection_validation(void)
{
    ...
    const nlo_vec_status vk_status = nlo_fft_plan_create_with_backend(backend,
                                                                      n,
                                                                      NLO_FFT_BACKEND_VKFFT,
                                                                      &vk_plan);
    assert(vk_status == NLO_VEC_STATUS_INVALID_ARGUMENT || vk_status == NLO_VEC_STATUS_UNSUPPORTED);
    assert(vk_plan == NULL);
...
static void test_fft_round_trip_cuda_if_available(void)
{
    ...
    for (size_t i = 0u; i < n; ++i) {
        time_domain[i] = nlo_make((double)i * 0.125, (double)(n - i) * 0.0625);
    }

    nlo_vector_backend* backend = nlo_vector_backend_create_cuda(NULL);
    if (backend == NULL) {
        printf("test_fft_round_trip_cuda_if_available: CUDA unavailable, skipping.\n");
        return;
    }
    ...
    for (size_t i = 0u; i < n; ++i) {
        assert(fabs(NLO_RE(round_trip[i]) - NLO_RE(time_domain[i])) < 1e-9);
        assert(fabs(NLO_IM(round_trip[i]) - NLO_IM(time_domain[i])) < 1e-9);
    }
    ...
    nlo_vec_destroy(backend, out);
    nlo_vector_backend_destroy(backend);
    printf("test_fft_round_trip_cuda_if_available: validates CUDA forward/inverse FFT consistency.\n");
#else
    printf("test_fft_round_trip_cuda_if_available: CUDA/cuFFT disabled, skipping.\n");
#endif
}
...
int main(void)
{
    ...
    test_fft_round_trip_cuda_if_available();
