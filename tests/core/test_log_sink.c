...
static int test_progress_callback(const progress_info* info, void* user_data)
{
...
{
    char text[64];
    (void)log_format_u64_grouped(text, sizeof(text), 0u);
    assert(strcmp(text, "0") == 0);
    (void)log_format_u64_grouped(text, sizeof(text), 24u);
    assert(strcmp(text, "24") == 0);
    (void)log_format_u64_grouped(text, sizeof(text), 60u);
    assert(strcmp(text, "60") == 0);
    (void)log_format_u64_grouped(text, sizeof(text), 1000u);
    assert(strcmp(text, "1,000") == 0);
    (void)log_format_u64_grouped(text, sizeof(text), 16384u);
    assert(strcmp(text, "16,384") == 0);
    (void)log_format_u64_grouped(text, sizeof(text), 2621440u);
    assert(strcmp(text, "2,621,440") == 0);
    (void)log_format_u64_grouped(text, sizeof(text), 1234567890ull);
...
{
    char text[64];
    (void)log_format_bytes_human(text, sizeof(text), 512u);
    assert(strcmp(text, "512 B") == 0);
    (void)log_format_bytes_human(text, sizeof(text), 2048u);
    assert(strcmp(text, "2.0 KB") == 0);
    (void)log_format_bytes_human(text, sizeof(text), 16384u);
    assert(strcmp(text, "16.0 KB") == 0);
    (void)log_format_bytes_human(text, sizeof(text), 2621440u);
    assert(strcmp(text, "2.5 MB") == 0);
    (void)log_format_bytes_human(text, sizeof(text), (size_t)(3u * 1024u * 1024u));
...
{
    assert(log_set_level(LOG_LEVEL_INFO) == 0);
    assert(log_set_buffer(8192u) == 0);
    assert(log_clear_buffer() == 0);

    log_emit(LOG_LEVEL_INFO, "[test] hello %d", 12345);

    char out[1024];
    size_t written = 0u;
    assert(log_read_buffer(out, sizeof(out), &written, 0) == 0);
    assert(written > 0u);
    assert(strstr(out, "hello 12345") != NULL);

    assert(log_read_buffer(out, sizeof(out), &written, 1) == 0);
    assert(written > 0u);
...
{
    assert(log_set_progress_stream(LOG_PROGRESS_STREAM_STDERR) == 0);
    assert(log_set_progress_stream(LOG_PROGRESS_STREAM_STDOUT) == 0);
    assert(log_set_progress_stream(LOG_PROGRESS_STREAM_BOTH) == 0);
    assert(log_set_progress_stream(-1) != 0);
    assert(log_set_progress_stream(3) != 0);
    assert(log_set_progress_stream(LOG_PROGRESS_STREAM_STDERR) == 0);
...
static void test_progress_entries(void)
{
    ...
    assert(log_clear_buffer() == 0);
    assert(log_set_level(LOG_LEVEL_INFO) == 0);
    assert(log_set_progress_options(1, 25, 1) == 0);

    log_progress_begin(0.0, 1.0);
    log_progress_step_rejected(0u, 0.0, 1.0, 0.1, 10.0, 0.02, 1u);
    log_progress_step_accepted(0u, 0.30, 1.0, 0.02, 0.5, 0.01);
    log_progress_finish(1.0, 1.0, 1);
...
static void test_progress_callback_abort(void)
{
    ...
    assert(log_set_progress_options(0, 25, 0) == 0);
    assert(log_set_progress_callback(test_progress_callback, &abort_after_first) == 0);

    log_progress_begin(0.0, 1.0);
    log_progress_step_accepted(0u, 0.25, 1.0, 0.02, 0.0, 0.02);
    assert(g_progress_callback_calls == 1);
    assert(log_progress_abort_requested() != 0);
    log_progress_finish(0.25, 1.0, 0);

    assert(log_set_progress_callback(NULL, NULL) == 0);
...
static void test_allocation_summary_marks_non_vulkan_fields_not_applicable(void)
{
    ...
    log_propagate_allocation_summary(VECTOR_BACKEND_AUTO,
                                         VECTOR_BACKEND_CPU,
                                         &allocation,
                                         NULL);
...
int main(void)
{
    ...
    (void)log_set_buffer(0u);
    (void)log_set_file(NULL, 0);
